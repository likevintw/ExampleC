...
//glassBrokenDetection R14 20190704_1759
...
void audio_detect::alert_initial()
{
...
void audio_detect::fft_array_initial()
{
	//
	alertPeriodPoint = alert_period * sampling_rate / data_size;
	alert_counter_threshold = alertPeriodPoint * 0.5;
...
//
void audio_detect::glassBrokenDetection()
{
	...
	//temp
	alert_frequency_minimum = 10000;
	alert_frequency_maximum = 22000;
	//
	alert_amplitude_threshold = 10000;
	


	for (int i(0),buf1(0); i < data_size / 2; i++)
	{
		...
		if (output_data_Amplitude[i] > alert_amplitude_threshold)
		{
			...
			if ((spectrum_x_axis[i] > alert_frequency_minimum) && (spectrum_x_axis[i] < alert_frequency_maximum))
			{
				...
				for (int k(0);; k++)
				{
					if (freList[k] == spectrum_x_axis[i]) break;
					if (freList[k] == 0)
					{
						freList[k] = spectrum_x_axis[i];
						break;
					}
				}
				*/
				...
				//
				buf1 = output_data_Amplitude[i] - lastSpectrum[i];
				if (buf1 > 100000)
				{
					if (spectrum_x_axis[i] != 0)
					{
						printf("%f\n", timePoint);
						//printf("%f  %d\n", timePoint, buf1);
						//printf("Frequency %f,  %d\n", spectrum_x_axis[i], buf1);
						//printf("Frequency %d\n", t3t4_1.spectrum_x_axis[i]);
					}
				}
				lastSpectrum[i] = output_data_Amplitude[i];
			}
...
//T3T4_detection
void audio_detect::T3T4_detection()
{
	//Show the start and end flag
	//printf("%d %d %f %f\n",fft_start_flag, fft_end_flag, float(fft_start_flag/16000), float(fft_end_flag / 16000));
	//buffer1 = alert_amplitude_threshold * 0.3;
...
//SQRT
int audio_detect::sqrt_Iterative(float input_value)
{
...
public:
	...
	void read_wav()
	{
		...
		wav_data_buffer = new short[wav_data_size];
		...
		if (wav_data_buffer[11] == 2)
		{
			...
			for (int i(24); i < wav_data_size; i++)
			{
	...
	//timeDomainPack
	float startAmplitude, endAmplitude, timeStartAmplitude, timeEndAmplitude;
...
void RW_wav::wavInfo()
{
	...
	if((wav_data_buffer[11] != 1)&& (wav_data_buffer[11] != 2))
	{
...
//
void RW_wav::timeDomainPack()
{
	startAmplitude = 0.5;
	endAmplitude = 0.1;
	timeStartAmplitude = 0;
	timeEndAmplitude = 0;
	float buffer1(0),buffer3(0);
	for (int i(23); i < wav_data_size-500; i++)
	{
		if ((wav_data_buffer[i] > 0)&&(wav_data_buffer[i] > wav_data_buffer[i + 1]) && (wav_data_buffer[i] > wav_data_buffer[i - 1]))
		{
			buffer1 = float(wav_data_buffer[i]) / 32767;
			if (buffer1 > startAmplitude)
			{
				timeStartAmplitude = 1;
				buffer3 = buffer1;
				timeStartAmplitude = i;
			}
			if (timeStartAmplitude != 0)
			{
				buffer3 = (buffer1 - buffer3) * 0.04+buffer3;
				//buffer3 = buffer1-1.3* buffer3;
				//printf("%f\n", buffer3);
			}
		}
		printf("%f\n", buffer3);
...
void T3T4_detection(std::string strFileName)
{
	...
	float deltaTime(0);
	float buffer2(0);
	deltaTime = float(t3t4_1.data_size) / float(t3t4_1.sampling_rate);
	for (int iseg = 0; iseg < iWavSegmentation; iseg++)
	{
		timePoint = iseg * deltaTime;
		//printf("Time Point:%f \n",iseg*deltaTime);
		//Get data of the fft_buffer
		for (int i(0), j(0); i < t3t4_1.data_size; i++)
		{
			t3t4_1.re[i] = float(wav_file_1.wav_data_buffer[iseg * t3t4_1.data_size + i])/32767;
		...
		//t3t4_1.FFT_2_radix();
		//t3t4_1.glassBrokenDetection();
		
		//show the result of FFT
		/*
		for (int i(0),buf1(0); i < t3t4_1.data_size/2; i++)
		{
			
			//printf("%f\n", lastSpectrum[i]);
			write_number_data_into_txt("outputData.txt", t3t4_1.output_data_Amplitude[i]);
			//lastSpectrum[i] = t3t4_1.output_data_Amplitude[i];
		}
		*/
...
int main()
{
	...
	for (int i(0); i < read1.fileList.size(); i++)
	{
		...
		if(passNumber==0)
		{ 
			printf("PassNumber: %d, PassRate:X\n", passNumber);
		}
		else
		{
			printf("PassNumber: %d, PassRate:%f\n", passNumber, frePassRate / passNumber);
		}
