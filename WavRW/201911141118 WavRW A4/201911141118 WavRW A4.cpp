...
// 201910301125 Wav Import and Export 
/*
Read Wav file >> OK
Write Wav File
*/
...
public:
	...
	// Convert txt to wav file
	void ConvertCharTxt2Wav();
	~WavRW();
};
WavRW::WavRW()
{
...
// txt file to wav
// input charater data
void CharTxt2Wav(std::string TxtFileName)
{
	// Define Class
	WavRW ExportWavFile1; 
	...
	// Set Import txt name
	// Open Import txt file
	// Get the data size of txt file
	// Set a buffer and load imported data

	// Set exported wav name
	TxtFileName = TxtFileName + ".wav";
	ExportWavFile1.ExportWavFileName = TxtFileName.c_str();
...
int main()
{
	...
	float floatBuffer(0);
	for (int i(-30000); i < 30000; i = i + 1000)
	{
		floatBuffer = i ;
		floatBuffer = floatBuffer / 32767;
		printf("%d,%f\n",i, floatBuffer);
	}
